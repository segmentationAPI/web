...
export function ProjectDashboard({ projectId }: { projectId: string }) {
	...
	const handleAutoLabel = async () => {
		if (!prompts.some(p => p.trim())) {
			toast.error(
				'Please configure at least one prompt before running Auto Label.',
			);
			setShowConfig(true);
			return;
		}
	...
	const handleDeleteSelected = async () => {
		...
		try {
			...
			if (!res.ok) {
				...
				toast.success(
					`Deleted ${selectedIds.size} image${selectedIds.size !== 1 ? 's' : ''}.`,
				);
				setSelectedIds(new Set());
				void fetchData();
			}
					...
					{activeJob && activeJob.totalItems > 0 && (
						<div className="absolute inset-0 z-0 bg-primary/10 w-full opacity-50">
							<div
								...
								style={{
									width: `${Math.round(((activeJob.successItems + activeJob.failedItems) / activeJob.totalItems) * 100)}%`,
								}}
							/>
						</div>
					)}
