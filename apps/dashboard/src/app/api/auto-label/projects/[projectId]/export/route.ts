  ...
  return new ReadableStream({
    start(controller) {
      archive.on("data", (chunk) => {
        if (!isCancelled) controller.enqueue(chunk);
      });
      archive.on("end", () => {
...
export async function GET(
  ...
  const [project] = await db
    .select({
      latestRequestId: autoLabelProject.latestRequestId,
  ...
  const processExport = async () => {
    try {
      ...
      if (formats.includes("labelstudio"))
        addLabelStudioFormat(archive, items, project.prompts ?? []);
    } finally {
      archive.finalize();
    }
  };
